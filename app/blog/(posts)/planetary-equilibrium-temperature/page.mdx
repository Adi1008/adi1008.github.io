...
<p className="text-sm text-zinc-400">March 24, 2023</p>
...
This is a consequence of conservation of energy, which states that energy cannot be created or destroyed. If we can derive expressions for $P_{\rm emitted}$ and $P_{\rm absorbed}$, we can set them equal to each other and solve for the equilibrium temperature of the planet. But, what are $P_{\rm emitted}$ and $P_{\rm absorbed}$?

## Finding $P_{\rm emitted}$

To find $P_{\rm emitted}$, we treat the planet as a black body of uniform temperature. The power emitted by the planet is, by definition, the luminosity of the planet, since we are neglecting any internal heat. Using the Stefan-Boltzmann Law, we find that it is

$$ P_{\rm emitted} = L_p = \boxed{4\pi R_p^2 \sigma T_p^4}$$
...
$$ P_{\rm absorbed} = \dfrac{\pi R_p^2R_s^2 \sigma T_s^4}{D^2} \times (1-\alpha)= \boxed{\dfrac{\pi R_p^2 R_s^2 \sigma T_s^4}{D^2}(1-\alpha)}$$

## Putting it all together

Now, we set $P_{\rm emitted}$ and $P_{\rm absorbed}$ equal to each other, since the planet is in thermal equilibrium, and solve for $T_p$:
